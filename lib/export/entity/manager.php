...
class Manager
{
	...
	protected static function loadCustomEntityList()
	{
		...
		$event = new Main\Event(Feed\Config::getModuleName(), 'onExportEntityTypeBuildList');
		$event->send();
		...
		foreach ($event->getResults() as $eventResult)
		{
			...
			if (isset($eventData['TYPE']))
			{
				if (
					...
					|| !is_subclass_of($eventData['SOURCE_CLASS_NAME'], 'Feed\Export\Entity\Reference\Source') // must be child of reference
				)
				{
				...
				if (
					...
					|| !is_subclass_of($eventData['EVENT_CLASS_NAME'], 'Feed\Export\Entity\Reference\Event') // must be child of reference
				)
				{
