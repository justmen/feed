...
namespace Ligacom\Feed\Export\Entity\Template;
...
class Source extends Feed\Export\Entity\Reference\Source
{
	...
	{
		if (Feed\Template\Engine::load())
		{
			$templateToKeyMap = [];

			foreach ($select as $templateKey => $template)
			{
				$templateNode = Feed\Template\Engine::compileTemplate($template);
	...
	public function getElementListValues($elementList, $parentList, $selectFields, $queryContext, $sourceValues)
	{
		...
		if (Feed\Template\Engine::load())
		{
			...
			{
				$entity = new Feed\Template\Entity\SourceValue($elementId);
				$result[$elementId] = [];
				...
				{
					/** @var Feed\Template\Node\Root $templateNode */
	...
	protected function setTemplateCache($template, Feed\Template\Node\Root $node)
	{
