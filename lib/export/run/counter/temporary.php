...
class Temporary extends Base
{
	...
	{
		return 'ligacom_feed_export_counter_element';
	}

	protected function getOfferTableName()
	{
		return 'ligacom_feed_export_counter_offer';
	}
	...
	protected function insertElements($filter, $context)
	{
		...
		else if ($context['OFFER_ONLY'])
		{
			...
			$elementSqlExtension['select'] = Feed\Export\Run\Steps\Offer::ELEMENT_TYPE_SKU . ', ' . $elementSqlExtension['select'];
		}
		else
		{
			$isNeedCatalogType = true;
		...
		else
		{
			$sql .= 'WHERE '
				. $sqlHelper->quote('CATALOG_TYPE') . ' NOT IN (' . implode(',', [
					Feed\Export\Run\Steps\Offer::ELEMENT_TYPE_SKU,
					Feed\Export\Run\Steps\Offer::ELEMENT_TYPE_EMPTY_SKU,
				]) . ')';
