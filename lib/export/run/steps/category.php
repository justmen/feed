...
class Category extends Base
{
	...
	{
		return Feed\Export\Run\Storage\CategoryTable::getClassName();
	}

	protected function getDataLogEntityType()
	{
		return Feed\Logger\Table::ENTITY_TYPE_EXPORT_RUN_CATEGORY;
	}
	...
	protected function getSectionListFromService($usedSectionIds, $context)
	{
		...
		$serviceCategoryList = Feed\Service\Data\Category::getList();
		$currentTree = [];
		$currentTreeDepth = 0;
				...
				foreach ($currentTree as $treeCategoryKey)
				{
					...
					if (!isset($result[$treeCategory['id']]))
					{
						$result[$treeCategory['id']] = [
							...
							'NAME' => Feed\Service\Data\Category::getTitle($treeCategory['id']),
						];
					}
				}
			}
	...
	protected function getSectionListFromIblock($usedSectionIds, $context)
	{
		...
		if (Main\Loader::includeModule('iblock'))
		{
			...
			while ($section = $querySections->Fetch())
			{
				$sectionData = [
					'ID' => (int)$section['ID'],
					'NAME' =>  preg_replace("/^[0-9\. ]*/","", trim($section['NAME'])),
				...
				else // get chain
				{
					...
					while ($parent = $queryParents->Fetch())
					{
						$parentData = [
							'ID' => (int)$parent['ID'],
							'NAME' => preg_replace("/^[0-9\. ]*/","", trim($parent['NAME'])),
		...
		foreach ($iblockLinkCollection as $iblockLink)
		{
			...
			if ($sourceMap === null)
			{
				throw new Main\SystemException(
					Feed\Config::getLang('EXPORT_RUN_STEP_CATEGORY_NOT_FOUND_SOURCE_FOR_TAG')
				);
			}

			$iblockContext = $iblockLink->getContext();
			$source = Feed\Export\Entity\Manager::getSource($sourceMap['TYPE']);

			if ($source->isVariable())
			{
				throw new Main\SystemException(
					Feed\Config::getLang('EXPORT_RUN_STEP_CATEGORY_NO_SUPPORT_FOR_VARIABLE_SOURCE')
				);
			}
			...
			{
				throw new Main\SystemException(
					Feed\Config::getLang('EXPORT_RUN_STEP_CATEGORY_NOT_FOUND_SOURCE_FIELD')
				);
			}
	...
	protected function getOfferTagSource(Feed\Export\IblockLink\Model $iblockLink, $tagName)
	{
		$result = null;
