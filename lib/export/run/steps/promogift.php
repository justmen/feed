...
class PromoGift extends Offer
{
    ...
    {
        return Feed\Export\Run\Manager::STEP_PROMO_GIFT;
    }
	...
	{
		return Feed\Logger\Table::ENTITY_TYPE_EXPORT_RUN_PROMO_GIFT;
	}
	...
	{
		return Feed\Export\Run\Storage\PromoGiftTable::getClassName();
	}
    ...
    protected function getIblockConfigList($isNeedAll = null)
    {
        ...
        if ($this->getRunAction() === 'change')
        {
            ...
            if (isset($changes[Feed\Export\Run\Manager::ENTITY_TYPE_PROMO]) && count($changes) === 1) // has only changes in promo
            {
	...
	protected function getExistOfferList($elementIdList, $context)
	{
		...
		{
			$queryExistOfferList = Feed\Export\Run\Storage\OfferTable::getList([
				'filter' => [
					'=SETUP_ID' => $context['SETUP_ID'],
