...
namespace Ligacom\Feed\Export\Run\Writer;
...
class File extends Base
{
	...
	public function writeTagList($elementList, $parentName, $position = null)
	{
		...
		if ($tagClosePosition !== null)
		{
			if ($isAfterSearch)
			{
				$tagClosePosition += Feed\Export\Run\Helper\BinaryString::getLength($searchName);
			}
	...
	public function updateTagList($tagName, $elementList, $idAttr = 'id', $isSelfClosed = null)
	{
		...
		if (!empty($searchList))
		{
			...
			$selfClose = '/>';
			$selfCloseLength = Feed\Export\Run\Helper\BinaryString::getLength($selfClose);
			$tagClose = '</' . $tagName . '>';
			$tagCloseLength = Feed\Export\Run\Helper\BinaryString::getLength($tagClose);
			$waitMergePositions = [];
			$waitMergeContents = [];
	...
	protected function writeSplice($startPosition, $finishPosition, $contents = '')
	{
		$resource = $this->getFileResource();
		$tempResource = null;
		$contentsLength = Feed\Export\Run\Helper\BinaryString::getLength($contents);
	...
	protected function getPositionList($searchList, $startPosition = null, $stopSearch = null)
	{
		...
		do
		{
			...
			foreach ($searchList as $searchKey => $searchVariant)
			{
				...
				{
					$variantPosition = Feed\Export\Run\Helper\BinaryString::getPosition($buffer, $searchVariant);
			...
			if ($stopSearch !== null)
			{
				$stopPosition = Feed\Export\Run\Helper\BinaryString::getPosition($buffer, $stopSearch);
	...
	protected function getFileResource()
	{
		if (!isset($this->fileResource))
		{
			...
			{
				throw new Main\SystemException(Feed\Config::getLang('EXPORT_RUN_WRITER_FILE_CANT_OPEN_FILE'));
			}
		}
	...
	protected function getTempResource()
	{
		...
		else
		{
			...
			{
				throw new Main\SystemException(Feed\Config::getLang('EXPORT_RUN_WRITER_FILE_CANT_OPEN_TEMP'));
			}
		}
	...
	protected function getBufferLength()
	{
		...
		{
			$this->bufferLength = (int)Feed\Config::getOption('export_run_writer_file_buffer_length');
	...
	protected function fileWrite($resource, $contents, $totalLength = null)
	{
		if ($totalLength === null)
		{
			$totalLength = Feed\Export\Run\Helper\BinaryString::getLength($contents);
		}
		...
		do
		{
			...
			if ($readyLength > 0)
			{
				$loopContents = Feed\Export\Run\Helper\BinaryString::getSubstring($contents, $readyLength, $totalLength);
			}
			...
			if ($loopLength === false)
			{
				throw new Main\SystemException(Feed\Config::getLang('EXPORT_RUN_WRITER_FILE_CANT_WRITE_FILE'));
			}
	...
	protected function streamCopy($fromResource, $toResource, $maxLength = null, $offset = null)
	{
		...
		if ($copyResult === false)
		{
			throw new Main\SystemException(Feed\Config::getLang('EXPORT_RUN_WRITER_STREAM_COPY_FAILED'));
		}
	}
}