...
class Weight extends Base
{
	...
	public function validate($value, array $context, $siblingsValues = null, Feed\Result\XmlNode $nodeResult = null, $settings = null)
	{
		...
		if (!$isEmptyValue)
		{
			...
			if ((float)$value <= 0)
			{
				...
				if ($nodeResult)
				{
					$nodeResult->registerError(Feed\Config::getLang($this->getLangKey() . '_VALIDATE_WEIGHT_NOT_POSITIVE'));
				}
			}
		}
	...
	public function getSettingsDescription()
	{
		...
		$result = [
			'BITRIX_UNIT' => [
				'TITLE' => Feed\Config::getLang($langKey . '_SETTINGS_BITRIX_UNIT_TITLE'),
				'TYPE' => 'enumeration',
				'VALUES' => []
			]
		];
		...
		foreach ($unitMap as $unit => $ratio)
		{
			$result['BITRIX_UNIT']['VALUES'][] = [
				'ID' => $unit,
				'VALUE' => Feed\Config::getLang($langKey . '_SETTINGS_BITRIX_UNIT_ENUM_' . strtoupper($unit))
			];
		}
