...
class BarcodeType extends StringType
{
    ...
    public function validate($value, array $context = [], Feed\Export\Xml\Reference\Node $node = null, Feed\Result\XmlNode $nodeResult = null)
    {
        ...
        if ($valueDigits === '')
        {
            ...
            if ($nodeResult)
            {
                $nodeResult->registerError(Feed\Config::getLang('TYPE_BARCODE_ERROR_NOT_NUMERIC'));
            }
        }
        else
        {
            ...
            if (isset(static::$availableLengthMap[$length]))
            {
                ...
                {
                    $nodeResult->registerError(Feed\Config::getLang('TYPE_BARCODE_ERROR_CHECKSUM_FAIL'));
                }
            }
            else
            {
                ...
                if ($nodeResult)
                {
                    $nodeResult->registerError(Feed\Config::getLang('TYPE_BARCODE_ERROR_NOT_FOUND_LENGTH_FORMAT'));
                }
            }
        }
