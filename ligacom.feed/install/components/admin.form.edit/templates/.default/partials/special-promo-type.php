<?php
if (!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED !== true) { die(); }

use Ligacom\Feed;

/** @var $component Ligacom\Feed\Components\AdminFormEdit */
/** @var $specialFields array */

$this->addExternalJs('/bitrix/js/ligacom.feed/ui/input/checktoggle.js');

$isPromoTypeGiftWithPurchase = false;

foreach ($specialFields as $specialFieldKey)
{
	$field = $component->getField($specialFieldKey);

	if ($field)
	{
	    // make gift iblock id required, if visible

	    if ($specialFieldKey === 'PROMO_GIFT_IBLOCK_ID')
        {
            $field['MANDATORY'] = 'Y';
        }

		$isPromoTypeField = ($specialFieldKey === 'PROMO_TYPE');

		if ($isPromoTypeField)
		{
			$isPromoTypeGiftWithPurchase = ($component->getFieldValue($field) === Feed\Export\Promo\Table::PROMO_TYPE_GIFT_WITH_PURCHASE);
		}

		if ($isPromoTypeField)
		{
			?>
			<tr class="js-plugin" data-plugin="Ui.Input.CheckToggle" data-input-element="select" data-target-element=".js-promo-type-target" data-value="<?= Feed\Export\Promo\Table::PROMO_TYPE_GIFT_WITH_PURCHASE; ?>">
			<?
		}
		else
		{
			?>
			<tr class="<?= $isPromoTypeGiftWithPurchase ? '' : 'is--hidden'; ?> js-promo-type-target">
			<?
		}
		?>
			<td width="40%" align="right" valign="top">
				<?
				include __DIR__ . '/field-title.php';
				?>
			</td>
			<td width="60%"><?= $component->getFieldHtml($field); ?></td>
		</tr>
		<?
	}
}
