...
if (!empty($arResult['ITEM']['PROMO_TYPE']))
{
	...
	$promoTypeLangTitle = Loc::getMessage('LIGACOM_FEED_T_ADMIN_FORM_EDIT_PROMO_TYPE_' . $promoTypeLangKey);
	$promoTypeLangDescription = Loc::getMessage('LIGACOM_FEED_T_ADMIN_FORM_EDIT_PROMO_TYPE_' . $promoTypeLangKey . '_DESCRIPTION');
	...
	if ($promoTypeLangTitle)
	{
		$promoTypeMessage = new CAdminMessage([
			...
			'DETAILS' => Loc::getMessage('LIGACOM_FEED_T_ADMIN_FORM_EDIT_PROMO_TYPE_' . $promoTypeLangKey . '_DESCRIPTION'),
			'HTML' => true
		]);
	}
}
