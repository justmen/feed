...
<td class="b-delivery-table__cell">
	<input class="is--persistent js-delivery-item__input" type="hidden" value="<?= Feed\Export\Delivery\Table::DELIVERY_TYPE_DELIVERY; ?>" data-name="DELIVERY_TYPE" <?
	...
	?> />
	<label>
		<input class="adm-designed-checkbox js-delivery-item__input <?= $arParams['MULTIPLE'] ? 'js-checkbox-radio__input' : ''; ?>" type="checkbox" data-name="DELIVERY_TYPE" value="<?= Feed\Export\Delivery\Table::DELIVERY_TYPE_PICKUP; ?>" <?

			if (!$isItemPlaceholder)
			{
				echo ' name="' . $itemInputName . '[DELIVERY_TYPE]"';

				if ($itemValue['DELIVERY_TYPE'] === Feed\Export\Delivery\Table::DELIVERY_TYPE_PICKUP)
				{
					echo ' checked';
				}
			}
