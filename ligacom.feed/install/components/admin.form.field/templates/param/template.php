...
$this->addExternalJs('/bitrix/js/ligacom.feed/source/manager.js');
$this->addExternalJs('/bitrix/js/ligacom.feed/field/param/tag.js');
$this->addExternalJs('/bitrix/js/ligacom.feed/field/param/tagcollection.js');
$this->addExternalJs('/bitrix/js/ligacom.feed/field/param/node.js');
$this->addExternalJs('/bitrix/js/ligacom.feed/field/param/nodecollection.js');

$lang = [
	'SELECT_PLACEHOLDER' => Loc::getMessage('LIGACOM_FEED_T_ADMIN_FIELD_PARAM_SELECT_PLACEHOLDER'),
	'PARAM_SIZE_NAME' => Loc::getMessage('LIGACOM_FEED_T_ADMIN_FIELD_PARAM_PARAM_SIZE_NAME'),
	'PARAM_SIZE_WARNINIG_REQUIRE_UNIT' => Loc::getMessage('LIGACOM_FEED_T_ADMIN_FIELD_PARAM_PARAM_SIZE_WARNINIG_REQUIRE_UNIT'),
];

$langStatic = [
	'HEADER_TAG' => Loc::getMessage('LIGACOM_FEED_T_ADMIN_FIELD_PARAM_HEADER_TAG'),
	'HEADER_SOURCE' => Loc::getMessage('LIGACOM_FEED_T_ADMIN_FIELD_PARAM_HEADER_SOURCE'),
	'HEADER_FIELD' => Loc::getMessage('LIGACOM_FEED_T_ADMIN_FIELD_PARAM_HEADER_FIELD'),
	'ADD_TAG' => Loc::getMessage('LIGACOM_FEED_T_ADMIN_FIELD_PARAM_ADD_TAG'),
	'SETTINGS_UTM_TOGGLE' => Loc::getMessage('LIGACOM_FEED_T_ADMIN_FIELD_PARAM_SETTINGS_UTM_TOGGLE'),
	'SETTINGS_UTM_TOGGLE_FILL' => Loc::getMessage('LIGACOM_FEED_T_ADMIN_FIELD_PARAM_SETTINGS_UTM_TOGGLE_FILL'),
	'SETTINGS_UTM_TOGGLE_ALT' => Loc::getMessage('LIGACOM_FEED_T_ADMIN_FIELD_PARAM_SETTINGS_UTM_TOGGLE_ALT'),
];
			...
			<div class="<?= $isTagPlaceholder ? 'is--hidden' : ''; ?> js-param-tag-collection__item" data-plugin="Field.Param.Tag" data-type="<?= $tagId; ?>" <?= $tag->isMultiple() ? 'data-multiple="true"' : ''; ?> <?= $tag->isRequired() ? 'data-required="true"' : ''; ?>>
				...
				<table class="js-param-tag__child" width="100%" data-plugin="Field.Param.NodeCollection" data-name="PARAM_VALUE">
					...
					if (!$tag->hasChildren())
					{
						...
						$attributeValue = [];
						$attributeType = Feed\Export\ParamValue\Table::XML_TYPE_VALUE;
					...
					if (!empty($attributes))
					{
						foreach ($attributes as $attribute)
						{
							...
							if ($isDefined && !$attribute->isVisible()) { continue; } /* ���������������� �������� */

							$attributeInputName = $tagInputName . '[PARAM_VALUE][' . $attributeIndex . ']';
							$attributeValue = null;
							$attributeId = $attribute->getId();
							$attributeName = $attribute->getName();
							$attributeType = Feed\Export\ParamValue\Table::XML_TYPE_ATTRIBUTE;
							...
							<tr class="<?= $hasActiveAddAttribute ? '' : 'is--hidden'; ?> js-param-node-collection__item-add-holder">
								...
								<td class="b-param-table__cell" colspan="3">
									<span class="js-params--show-hidden-tags">
										<a href="javascript:void(0)"><?= Loc::getMessage('LIGACOM_FEED_T_ADMIN_FIELD_PARAM_ADD_ATTRIBUTE', [
											'#TAG_NAME#' => $tagName
										]); ?></a>
				...
				<td class="b-param-table__cell">
					...
					if (!empty($arResult['DOCUMENTATION_LINK']))
					{
						...
						<div class="b-admin-message-list spacing--1x2">
							...
							\CAdminMessage::ShowMessage([
								'TYPE' => 'OK',
								'MESSAGE' => Loc::getMessage('LIGACOM_FEED_T_ADMIN_FIELD_PARAM_DOCUMENTATION_TITLE'),
								'DETAILS' => Loc::getMessage('LIGACOM_FEED_T_ADMIN_FIELD_PARAM_DOCUMENTATION_DETAILS', [
					...
					if (!empty($arResult['DOCUMENTATION_BETA']))
					{
						...
						<div class="b-admin-message-list">
							...
							\CAdminMessage::ShowMessage([
								'TYPE' => 'ERROR',
								'MESSAGE' => Loc::getMessage('LIGACOM_FEED_T_ADMIN_FIELD_PARAM_DOCUMENTATION_BETA', [
...
<script>
	(function() {
		...
		new Source.Manager('#<?= $fieldId ?>', <?= Ligacom\Feed\Utils::jsonEncode($managerData, JSON_UNESCAPED_UNICODE); ?>);

		// extend lang

		utils.registerLang(<?= Ligacom\Feed\Utils::jsonEncode($lang, JSON_UNESCAPED_UNICODE); ?>, 'LIGACOM_FEED_FIELD_PARAM_');
	})();
</script>