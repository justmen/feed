...
namespace Ligacom\Feed\Component\Promo;
...
class EditForm extends Feed\Component\Model\EditForm
{
	...
	protected function validateSetupSelected(Main\Entity\Result $result, $data, $fields)
	{
		...
		if (isset($fields['SETUP_EXPORT_ALL']))
		{
			$hasEmptySetupExportAll = (
				...
				|| (string)$data['SETUP_EXPORT_ALL'] === Feed\Export\Promo\Table::BOOLEAN_N
			);
		}
		...
		{
			$result->addError(new Main\Entity\EntityError(
				Feed\Config::getLang('COMPONENT_PROMO_EDIT_FORM_FIELD_SETUP_LINK_EMPTY'),
				Main\Entity\FieldError::EMPTY_REQUIRED
			));
		}
	}
	...
	protected function validateProductFilter(Main\Entity\Result $result, $data, $fields)
	{
		...
		foreach ($productFieldKeys as $productFieldKey)
		{
			if (isset($fields[$productFieldKey]))
			{
							...
							foreach ($promoProduct['FILTER'] as $filterIndex => $filter)
							{
								...
								if (!$hasValidCondition)
								{
									$result->addError(new Feed\Error\EntityError(
										Feed\Config::getLang('COMPONENT_PROMO_EDIT_FORM_ERROR_FILTER_CONDITION_EMPTY', [
				...
				{
					$result->addError(new Feed\Error\EntityError(
						Feed\Config::getLang('COMPONENT_PROMO_EDIT_FORM_ERROR_PRODUCT_EXPORT_EMPTY', [
							'#FIELD_NAME#' => $field['LIST_COLUMN_LABEL']
						])
					));
				}
	...
	public function getFields(array $select = [], $item = null)
	{
		...
		if (in_array('PROMO_GIFT_IBLOCK_ID', $select) && !isset($result['PROMO_GIFT_IBLOCK_ID']))
		{
			$result['PROMO_GIFT_IBLOCK_ID'] = [
				...
				'EDIT_IN_LIST' => 'Y',
				'LIST_COLUMN_LABEL' => Feed\Config::getLang('COMPONENT_PROMO_EDIT_FORM_FIELD_PROMO_GIFT_IBLOCK_ID'),
				'USER_TYPE' => $USER_FIELD_MANAGER->GetUserType('enumeration'),
				'MULTIPLE' => 'N',
				'MANDATORY' => 'N'
			];

			$result['PROMO_GIFT_IBLOCK_ID']['USER_TYPE']['CLASS_NAME'] = 'Ligacom\Feed\Ui\UserField\IblockType';
 		}
	...
	public function extend($data, array $select = [])
	{
		...
		foreach ($productFieldKeys as $productFieldKey)
		{
			if (!empty($result[$productFieldKey]))
			{
				...
				{
					$promoProduct['CONTEXT'] = Feed\Export\Entity\Iblock\Provider::getContext($promoProduct['IBLOCK_ID']);
				}
				unset($promoProduct);
			}
		}
	...
	protected function getUsedIblockList($data)
	{
		...
		$querySetupIblockLinkList = Feed\Export\IblockLink\Table::getList([
	...
	protected function getSetupFilter($data, $reference = null)
	{
		...
		if (isset($data['SETUP_EXPORT_ALL']) && $data['SETUP_EXPORT_ALL'] === Feed\Export\Promo\Table::BOOLEAN_Y)
		{
			// nothing
		}
		else
	...
	protected function getTypeDiscountRuleFields($promoType)
	{
		...
		switch ($promoType)
		{
			case Feed\Export\Promo\Table::PROMO_TYPE_FLASH_DISCOUNT:
				$result = [
					'START_DATE',
					'FINISH_DATE',
					'DISCOUNT_UNIT',
			...
			case Feed\Export\Promo\Table::PROMO_TYPE_GIFT_N_PLUS_M:
				$result = [
					'START_DATE',
					'FINISH_DATE',
					'PROMO_PRODUCT',
	...
	protected function getTypeDiscountOverrides($promoType)
	{
		...
		switch ($promoType)
		{
			...
			default:
				...
				if (!Feed\Export\Promo\Discount\Manager::isInternalType($promoType))
                {
                    try
                    {
                        $providerClassName = Feed\Export\Promo\Discount\Manager::getProviderTypeClassName($promoType);
                        ...
                        if ($providerEnum !== null)
                        {
                            ...
                            $result['EXTERNAL_ID']['USER_TYPE']['CLASS_NAME'] = 'Ligacom\Feed\Ui\UserField\EnumerationType';
