...
class GridList extends Feed\Component\Model\GridList
{
	public function getFields(array $select = [])
	{
		...
		{
			$result['SETUP']['LIST_COLUMN_LABEL'] = Feed\Config::getLang('COMPONENT_PROMO_GRID_LIST_FIELD_SETUP_WITH_STATUS');
	...
	public function load(array $queryParameters = [])
    {
        ...
        foreach ($result as &$item)
        {
            if (
				...
				&& (string)$item['SETUP_EXPORT_ALL'] === Feed\Export\Promo\Table::BOOLEAN_Y // export for all
			)
            {
    ...
    protected function loadExportStatus(&$result)
	{
		...
		{
			$promo = Feed\Export\Promo\Model::initialize($row);
			$exportStatus = [
				'STATUS' => true,
				'REASON' => null,
				'RESULT' => []
			...
			if (!$promo->isActive())
			{
				...
				$exportStatus['REASON'] = Feed\Config::getLang('COMPONENT_PROMO_GRID_LIST_EXPORT_REASON_INACTIVE');
			}
			else if (!$promo->isActiveDate())
			{
				...
				if ($nextDate)
				{
					$exportStatus['REASON'] = Feed\Config::getLang('COMPONENT_PROMO_GRID_LIST_EXPORT_REASON_FUTURE', [
						'#DATE#' => $nextDate
					]);
				}
				else
				{
					$exportStatus['REASON'] = Feed\Config::getLang('COMPONENT_PROMO_GRID_LIST_EXPORT_REASON_PAST');
				}
			}
		...
		if (!empty($rowMap))
		{
			$queryExportResult = Feed\Export\Run\Storage\PromoTable::getList([
	...
	public function filterActions($item, $actions)
    {
        ...
        foreach ($result as $actionKey => $action)
        {
            ...
            {
                case 'ACTIVATE':
                    $isValid = ($item['ACTIVE'] === Feed\Export\Promo\Table::BOOLEAN_N);
                break;

                case 'DEACTIVATE':
                    $isValid = ($item['ACTIVE'] === Feed\Export\Promo\Table::BOOLEAN_Y);
                break;
            }
    ...
    protected function activateItem($id)
    {
        ...
        $dataClass::update($id, [
            'ACTIVE' => Feed\Export\Promo\Table::BOOLEAN_Y
        ]);
    }
    ...
    protected function deactivateItem($id)
    {
        ...
        $dataClass::update($id, [
            'ACTIVE' => Feed\Export\Promo\Table::BOOLEAN_N
        ]);
    }
}