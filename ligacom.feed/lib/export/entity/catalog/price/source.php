...
namespace Ligacom\Feed\Export\Entity\Catalog\Price;
...
class Source extends Feed\Export\Entity\Reference\Source
{
	...
	public function initializeQueryContext($select, &$queryContext, &$sourceSelect)
	{
		...
		if ($this->hasQueryDiscountValue($select, $queryContext))
		{
			...
			if ($this->isNeedDiscountCache($select, $queryContext))
			{
				...
				$elementPropertyType = Feed\Export\Entity\Manager::TYPE_IBLOCK_ELEMENT_PROPERTY;
				$offerPropertyType = Feed\Export\Entity\Manager::TYPE_IBLOCK_OFFER_PROPERTY;
	...
	public function getFields(array $context = [])
	{
		...
		if ($context['HAS_CATALOG'] && Main\Loader::includeModule('catalog'))
		{
			...
			$priceTypes = [
				'MINIMAL' => [
					'VALUE' => Feed\Config::getLang($langPrefix . 'TYPE_MINIMAL'),
					'FILTERABLE' => false,
					'SELECTABLE' => true
				],
				'BASE' => [
					'VALUE' => Feed\Config::getLang($langPrefix . 'TYPE_BASE'),
					'FILTERABLE' => true,
					'SELECTABLE' => true
				],
				'OPTIMAL' => [
					'VALUE' => Feed\Config::getLang($langPrefix . 'TYPE_OPTIMAL'),
					'FILTERABLE' => false,
					'SELECTABLE' => true
				]
			];
			...
			$priceFields = [
				'DISCOUNT_VALUE' => [
					'VALUE' => Feed\Config::getLang($langPrefix . 'FIELD_DISCOUNT_VALUE'),
					'TYPE' => Feed\Export\Entity\Data::TYPE_NUMBER,
					'FILTERABLE' => false,
					'SELECTABLE' => true
				],
				'VALUE' => [
					'VALUE' => Feed\Config::getLang($langPrefix . 'FIELD_VALUE'),
					'TYPE' => Feed\Export\Entity\Data::TYPE_NUMBER,
					'FILTERABLE' => true,
					'SELECTABLE' => true
				],
				'CURRENCY' => [
					'VALUE' => Feed\Config::getLang($langPrefix . 'FIELD_CURRENCY'),
					'TYPE' => Feed\Export\Entity\Data::TYPE_CURRENCY,
					'FILTERABLE' => true,
					'SELECTABLE' => true
				]
			];
	...
	{
		return Feed\Config::getOption('export_catalog_price_discount_properties_optimize', 'Y') === 'Y';
	}
	...
	protected function hasGetOptimalPriceHandlers()
	{
		...
		{
			$option = Feed\Config::getOption('export_catalog_price_ignore_optimal_handlers', 'N');
