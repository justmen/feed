...
namespace Ligacom\Feed\Export\Entity;
...
class Data
{
	...
	public static function getType($type)
	{
		...
		if (!isset($typeList[$type]))
		{
			...
			throw new Main\SystemException(
				Feed\Config::getLang($langPrefix . 'TYPE_UNDEFINED')
			);
		}
	...
	{
		$stringDataTypes = [
			Feed\Type\Manager::TYPE_STRING,
			Feed\Type\Manager::TYPE_NUMBER,
			Feed\Type\Manager::TYPE_VAT,
			Feed\Type\Manager::TYPE_BARCODE,
			Feed\Type\Manager::TYPE_HTML,
			Feed\Type\Manager::TYPE_DATE,
			Feed\Type\Manager::TYPE_DATEPERIOD,
			Feed\Type\Manager::TYPE_CURRENCY,
			Feed\Type\Manager::TYPE_BOOLEAN,
			Feed\Type\Manager::TYPE_AGE,
			Feed\Type\Manager::TYPE_DIMENSIONS,
			Feed\Type\Manager::TYPE_RECORDING_LENGTH,
			Feed\Type\Manager::TYPE_CONDITION,
		];
		...
		return [
			...
			static::TYPE_IBLOCK_SECTION => [
				'DATA' => [
					Feed\Type\Manager::TYPE_CATEGORY
				],
				'COMPARE' => [
					static::COMPARE_IN,
					static::COMPARE_NOT_IN,
			...
			static::TYPE_DELIVERY_OPTIONS => [
				'DATA' => [
					Feed\Type\Manager::TYPE_DELIVERY_OPTIONS
				],
				'COMPARE' => []
			]
		];
	...
	public static function getCompare($compare)
	{
		...
		if (!isset($compareList[$compare]))
		{
			...
			throw new Main\SystemException(
				Feed\Config::getLang($langPrefix . 'COMPARE_UNDEFINED')
			);
		}
	...
	public static function getCompareTitle($compare)
	{
		...
		return Feed\Config::getLang($langPrefix . 'COMPARE_' . strtoupper($compare));
	}
