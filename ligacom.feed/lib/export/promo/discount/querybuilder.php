...
class QueryBuilder
{
	...
	protected static function getConditionIblockList($condition)
	{
		...
		if ($condition['FIELD']['TYPE'] === Feed\Export\Entity\Manager::TYPE_IBLOCK_ELEMENT_FIELD)
		{
        ...
        if (Main\Loader::includeModule('catalog'))
        {
            ...
            if (!empty($controls) && is_array($controls))
            {
                foreach ($controls as $classId => $control)
                {
                    ...
                    {
                        $sourceType = Feed\Export\Entity\Manager::TYPE_IBLOCK_ELEMENT_FIELD;
                        ...
                        if (isset($control['ENTITY']))
                        {
                            ...
                            {
                                case 'PRODUCT':
                                    $sourceType = Feed\Export\Entity\Manager::TYPE_CATALOG_PRODUCT;
                                    $hasParent = false;
                                    $parentLogic = null;
                                break;
                            }
            ...
            if ($isLogicAnd && !empty($elementFilter))
            {
                ...
                if ($hasMultipleSourceLogicAnd || count($multipleSourceConditionList) > 1)
                {
                    foreach ($multipleSourceConditionList as $multipleSourceCondition)
                    {
                        ...
                        $elementConditionSource = Feed\Export\Entity\Manager::TYPE_IBLOCK_OFFER_PROPERTY;
    ...
    protected static function convertElementSourceToOffer($sourceType)
    {
        ...
        switch ($sourceType)
        {
            case Feed\Export\Entity\Manager::TYPE_IBLOCK_ELEMENT_FIELD:
                $result = Feed\Export\Entity\Manager::TYPE_IBLOCK_OFFER_FIELD;
            break;

            case Feed\Export\Entity\Manager::TYPE_IBLOCK_ELEMENT_PROPERTY:
                $result = Feed\Export\Entity\Manager::TYPE_IBLOCK_OFFER_PROPERTY;
            break;
        }
