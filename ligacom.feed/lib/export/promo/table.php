...
class Table extends Feed\Reference\Storage\Table
{
	...
	{
		return 'ligacom_feed_export_promo';
	}
	...
	public static function getMap()
	{
		return [
			...
			new Main\Entity\ReferenceField('PROMO_PRODUCT', Feed\Export\PromoProduct\Table::getClassName(), [
				'=this.ID' => 'ref.PROMO_ID'
			]),

			new Main\Entity\ReferenceField('PROMO_GIFT', Feed\Export\PromoGift\Table::getClassName(), [
				'=this.ID' => 'ref.PROMO_ID'
			])
		];
	}
	...
	public static function getReference($primary = null)
	{
		return [
			...
			'PROMO_PRODUCT' => [
				'TABLE' => Feed\Export\PromoProduct\Table::getClassName(),
				'LINK_FIELD' => 'PROMO_ID',
				'LINK' => [
					'PROMO_ID' => $primary
				]
			...
			'PROMO_GIFT' => [
				'TABLE' => Feed\Export\PromoGift\Table::getClassName(),
				'LINK_FIELD' => 'PROMO_ID',
				'LINK' => [
					'PROMO_ID' => $primary
				]
	...
	public static function getMapDescription()
	{
        ...
        if (isset($result['PROMO_TYPE']))
        {
            ...
            $result['PROMO_TYPE']['USER_TYPE']['CLASS_NAME'] = 'Ligacom\Feed\Ui\UserField\EnumerationType';
            $result['PROMO_TYPE']['VALUES'] = Discount\Manager::getTypeEnum();
        }

		if (isset($result['DESCRIPTION']))
		{
			$result['DESCRIPTION']['USER_TYPE']['CLASS_NAME'] = 'Ligacom\Feed\Ui\UserField\HtmlType';
		}

		if (isset($result['SETUP']))
		{
			$result['SETUP']['MULTIPLE'] = 'Y';
			$result['SETUP']['USER_TYPE']['CLASS_NAME'] = 'Ligacom\Feed\Ui\UserField\SetupLinkType';
		}
        ...
        if (isset($result['DISCOUNT_CURRENCY']) && Main\Loader::includeModule('currency'))
        {
            ...
            if (!empty($currencyList))
            {
                ...
                $result['DISCOUNT_CURRENCY']['USER_TYPE']['CLASS_NAME'] = 'Ligacom\Feed\Ui\UserField\EnumerationType';
	...
	public static function validateUrl()
	{
		return [
			function ($value, $primary, $row, $field)
            {
		        ...
		        if (!$result)
                {
                    $result = Feed\Config::getLang('EXPORT_PROMO_VALIDATE_URL_REQUIRE_PATH');
                }
        ...
        return [
            function($value, $primary, $row, $field)
			{
				...
				if ($valueString !== '')
				{
					...
					{
						$result = Feed\Config::getLang('EXPORT_PROMO_VALIDATE_POSITIVE_INTEGER_NOT_NUMBER', [
							'#NAME#' => $field->getTitle()
						]);
					}
					else if ($valueInteger <= 0)
					{
						$result = Feed\Config::getLang('EXPORT_PROMO_VALIDATE_POSITIVE_INTEGER_NOT_POSITIVE', [
							'#NAME#' => $field->getTitle()
						]);
					}
				}
        ...
        return [
            function($value, $primary, $row, $field)
			{
				...
				if ($valueString !== '')
				{
					...
					{
						$result = Feed\Config::getLang('EXPORT_PROMO_VALIDATE_POSITIVE_FLOAT_NOT_NUMBER', [
							'#NAME#' => $field->getTitle()
						]);
					}
					else if ($valueFloat <= 0)
					{
						$result = Feed\Config::getLang('EXPORT_PROMO_VALIDATE_POSITIVE_FLOAT_NOT_POSITIVE', [
							'#NAME#' => $field->getTitle()
						]);
					}
				}
    ...
    public static function validateStartDate()
    {
        return [
            function ($value, $primary, $row, $field)
            {
                ...
                {
                    $result = Feed\Config::getLang('EXPORT_PROMO_VALIDATE_START_DATE_MUST_LESS_FINISH_DATE');
                }
