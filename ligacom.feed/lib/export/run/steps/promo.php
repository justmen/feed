...
class Promo extends Base
{
    ...
    {
        return Feed\Export\Run\Manager::STEP_PROMO;
    }
    ...
    public function run($action, $offset = null)
    {
        ...
        if ($action === 'change')
        {
            ...
            if (isset($changes[Feed\Export\Run\Manager::ENTITY_TYPE_PROMO])) // has only changes in promo
            {
				$isOnlyPromoChanged = (count($changes) === 1);

                $promoChangedMap += array_flip($changes[Feed\Export\Run\Manager::ENTITY_TYPE_PROMO]);
            }
    ...
    {
        return Feed\Logger\Table::ENTITY_TYPE_EXPORT_RUN_PROMO;
    }

    protected function getStorageDataClass()
    {
        return Feed\Export\Run\Storage\PromoTable::getClassName();
    }

    protected function getStorageRuntime()
    {
        return [
            new Main\Entity\ReferenceField('EXPORT_PROMO_PRODUCT', Feed\Export\Run\Storage\PromoProductTable::getClassName(), [
     ...
     * �������� ����������� ����������� ������� ��� promo
     *
     * @param Feed\Export\Promo\Model $promo
     * @param $context
     *
     * @return array
     */
    protected function getExportPromoProductList(Feed\Export\Promo\Model $promo, $context)
    {
        $result = [];

        $queryExportProductList = Feed\Export\Run\Storage\PromoProductTable::getList([
            'filter' => [
                '=SETUP_ID' => $context['SETUP_ID'],
     ...
     */
    protected function getExportPromoGiftList(Feed\Export\Promo\Model $promo, $context)
    {
        $result = [];

        $queryExportProductList = Feed\Export\Run\Storage\PromoGiftTable::getList([
            'filter' => [
                '=SETUP_ID' => $context['SETUP_ID'],
                '=PROMO_ID' => $promo->getId(),
                '=STATUS' => static::STORAGE_STATUS_SUCCESS,
                [
                    'LOGIC' => 'OR',
                    [ '=ELEMENT_TYPE' => Feed\Export\PromoGift\Table::PROMO_GIFT_TYPE_OFFER ],
                    [
            ...
            'runtime' => [
                new Main\Entity\ReferenceField('GIFT_EXPORT', Feed\Export\Run\Storage\GiftTable::getClassName(), [
                    '=this.SETUP_ID' => 'ref.SETUP_ID',
    ...
    {
        return (int)($this->getParameter('promoPageSize') ?: Feed\Config::getOption('export_run_promo_page_size') ?: 20);
    }
