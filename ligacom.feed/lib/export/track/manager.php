...
class Manager
{
    ...
    public static function registerEntityChange($entityType, $entityId)
    {
        ...
        {
            Feed\Export\Run\Manager::registerChange($setupId, $setupElementType, $entityId);
        }
    }
    ...
    public static function registerElementChange($elementType, $elementId, $sourceType, $sourceParams = null)
    {
        ...
        if (!isset(static::$registeredElementChanges[$changeKey]))
        {
            ...
            foreach ($setupList as $setupId => $setupElementType)
            {
                Feed\Export\Run\Manager::registerChange($setupId, $setupElementType, $elementId);
            }
        }
    }
                ...
                case Table::ENTITY_TYPE_PROMO:
                    ...
                    foreach ($promoSetupList as $setupId)
                    {
                        if (!isset($result[$setupId]))
                        {
                            ...
                            if ($elementType === Feed\Export\Run\Manager::ENTITY_TYPE_OFFER)
                            {
    ...
    protected static function convertEntityToSetupList($entityType, $entityId)
    {
        ...
        switch ($entityType)
        {
            case Table::ENTITY_TYPE_PROMO:
                ...
                foreach ($promoSetupList as $setupId)
                {
                    $result[$setupId] = Feed\Export\Run\Manager::ENTITY_TYPE_PROMO;
                }
            break;
        }
    ...
    protected static function getPromoSetupList($promoId)
    {
        ...
        else
        {
            ...
            $queryPromo = Feed\Export\Promo\Table::getList([
                'filter' => [ '=ID' => $promoId ],
            ...
            {
                if ($promo['SETUP_EXPORT_ALL'] === Feed\Export\Promo\Table::BOOLEAN_Y)
                {
                ...
                {
                    $querySetupLinkList = Feed\Export\Promo\Internals\SetupLinkTable::getList([
                        'filter' => [
                            '=PROMO_ID' => $promoId,
                            '=SETUP.AUTOUPDATE' => '1'
                        ],
    ...
    protected static function resolveSetupElementTypeList($setupList, $elementType, $sourceType, $sourceParams)
    {
        ...
        if ($elementType === Feed\Export\Run\Manager::ENTITY_TYPE_OFFER && !empty($sourceParams['IBLOCK_ID']))
        {
            ...
            foreach ($setupList as $setupId)
            {
                ...
                {
                    $result[$setupId] = Feed\Export\Run\Manager::ENTITY_TYPE_GIFT;
                }
            }
        }
    ...
    protected static function getSetupIblockList($setupList)
    {
        ...
        if (!empty($needFetch))
        {
            $queryIblockLinkList = Feed\Export\IblockLink\Table::getList([
