...
class Property
{
	const USER_TYPE = 'lg_condition_type';
	...
	public static function GetUserTypeDescription()
	{
		...
		return [
			...
			'DESCRIPTION' => Feed\Config::getLang($langKey . 'DESCRIPTION'),
	...
	public static function GetPropertyFieldHtml($property, $value, $controlName)
	{
		...
		{
			$result .= '<option value="">' . Feed\Config::getLang($langKey . 'VALUE_EMPTY') . '</option>';
		}
		...
		if ($property['WITH_DESCRIPTION'] === 'Y')
		{
			$result .=
				'&nbsp;<span>'
				. Feed\Config::getLang($langKey . 'VALUE_DESCRIPTION') . ':&nbsp;'
	...
	public static function GetAdminFilterHTML($property, $controlName)
	{
		...
		$result = '<select name="'.$controlName['VALUE'].'[]">';
		$result .= '<option value="">' . Feed\Config::getLang($langKey . 'VALUE_ANY') . '</option>';
	...
	{
		return [
			Feed\Type\ConditionType::TYPE_LIKE_NEW,
			Feed\Type\ConditionType::TYPE_USED
		];
	}
	...
	public static function GetOptionLabel($option)
	{
		...
		return Feed\Config::getLang($langKey . 'OPTION_' . strtoupper($option));
	}
